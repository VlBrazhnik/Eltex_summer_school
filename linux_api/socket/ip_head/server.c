...
main(int argc, char **argv)
{
    ...
    srvaddr.sin_addr.s_addr = inet_addr("127.0.0.2");
    ...
    while(1) { 
        ...
        char *srvmsg = "IM SERVER";
        memmove(clmsg, srvmsg, strlen(srvmsg));

        numRead = sendto(sfd, clmsg, PACKAGE_SIZE - sizeof(uHead), 0, 
                        (struct sockaddr *) &claddr, len);
        if (numRead == -1) {
            perror("send");
            exit(EXIT_FAILURE);
        } else {
            printf("[SERVER_SENT_BYTES]: %d\n", (int)numRead);
            printf("[SERVER_SENT_MESG]: %s\n", clmsg);
            break;
        }
    }
    free(clmsg);
